...
def create_lattice(sent):
	...
	for i_e in range(len(sent))[1:]: #index_end
		for i_b in range(i_e): #index_begin
			str_part = sent[i_b:i_e] #文字の塊 
			lattice[i_b][i_e] = create_nodes(str_part)
	return lattice
...
def get_wv_cost(w_surface, v_surface, wv_costs_dict):
	return wv_costs_dict[w_surface][v_surface]
...
def main():
	...
	word_dict = pickle.load(open("ipadic_word_dict", "r"))
	read_pron_dict = pickle.load(open("ipadic_read_pron_word_dict", "r"))
	id_def = load_2colums(open(dict_dir + "left-id.def","r"))

	#costs_dict
	#? vc_cost_dict =  mecab辞書中のコストで良い？
	...
	lattice = create_lattice(input_sent, wv_costs_dict)
	result = morpheme_analizer(lattice, cc_costs_dict, id_def)
